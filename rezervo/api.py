...
from rezervo.schemas.booking import BookingPayload
...
from rezervo.schemas.config.stored import StoredConfig
from rezervo.schemas.config.user import UserConfig, UserNameWithIsSelf
from rezervo.schemas.session import UserNameSessionStatus
from rezervo.settings import Settings, get_settings
from rezervo.types import CancelBookingActionValue, Interaction
from rezervo.utils.config_utils import class_config_recurrent_id
from rezervo.utils.cron_utils import (
    build_cron_comment_prefix_for_config,
    build_cron_jobs_from_config,
    upsert_jobs_by_comment,
)
from rezervo.utils.ical_utils import ical_event_from_sit_class_session
...
def find_config_by_slack_id(
    configs: list[ConfigValue], user_id: str
...
def handle_cancel_booking_slack_action(
    ...
    if config.notifications is None:
        print(
            "[WARNING] Notifications config not specified, no notifications will be sent!"
        )
        slack_config = None
    else:
        ...
        if slack_config is None:
            print(
                "[WARNING] Slack notifications config not specified, no Slack notifications will be sent!"
            )
        else:
            notify_working_slack(
    ...
    if isinstance(auth_result, AuthenticationError):
        ...
        if slack_config is not None:
            notify_cancellation_failure_slack(
                slack_config.bot_token, slack_config.channel_id, message_ts, auth_result
            )
        return
    ...
    if _class is None:
        ...
        return
    cancellation_error = try_cancel_booking(
        auth_result, _class, config.booking.max_attempts
    )
...
async def slack_action(
    ...
    if action.action_id == SLACK_ACTION_CANCEL_BOOKING:
        configs: list[ConfigValue] = [
            config_from_stored(StoredConfig.from_orm(c)).config
...
async def book_class(
    ...
    config = config_from_stored(
        StoredConfig.from_orm(crud.get_user_config(db, db_user.id))
    ).config
    print("[INFO] Authenticating...")
    booking_token = try_authenticate(
        config.auth.email, config.auth.password, config.auth.max_attempts
    )
    if isinstance(booking_token, AuthenticationError):
        ...
        return Response(
            ...
            content="Authentication failed for sit.no",
        )
    ...
    if _class is None:
        ...
        return Response(
            ...
            content="Class not found for given class id",
        )
    booking_result = try_book_class(
        booking_token, _class, config.booking.max_attempts, config.notifications
    )
...
def cancel_booking(
    ...
    print("[INFO] Authenticating...")
    cancellation_token = try_authenticate(
        config.auth.email, config.auth.password, config.auth.max_attempts
    )
    ...
    cancellation_error = try_cancel_booking(
        cancellation_token, _class, config.booking.max_attempts
    )
    if cancellation_error is not None:
        return Response(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)
    pull_sessions(db_user.id)
...
def get_all_configs_index(
    ...
    user_name_lookup = {
        u.id: auth0_mgmt_client.users.get(u.jwt_sub)["name"] for u in db_users
    }
    user_configs_dict = {}
    for dbu in db_users:
        db_user_config: Optional[models.Config] = (
            db.query(models.Config).filter_by(user_id=dbu.id).one_or_none()
        )
        ...
        for c in user_config.classes:
            ...
            user_configs_dict[class_id].append(
                UserNameWithIsSelf(
                    is_self=dbu.id == db_user.id, user_name=user_name_lookup[dbu.id]
                )
            )
...
def get_sessions_index(
    ...
    for session in (
        ...
        .filter(models.Session.status != models.SessionState.PLANNED)
        .all()
    ):
