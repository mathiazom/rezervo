...
from sit_rezervo.consts import CLASSES_SCHEDULE_URL, CLASSES_SCHEDULE_DAYS_IN_SINGLE_BATCH
from sit_rezervo.schemas.schedule import SitSchedule, SitDay


def fetch_single_batch_sit_schedule(token, studio: str = None, from_iso: str = None) -> Union[SitSchedule, None]:
    res = requests.get(
...
def fetch_sit_schedule(token, studio: str = None, days: int = None) -> Union[SitSchedule, None]:
    schedule_days: list[SitDay] = []
    from_date = datetime.datetime.now().date()
    for i in range(math.ceil(days / CLASSES_SCHEDULE_DAYS_IN_SINGLE_BATCH) if days is not None else 1):
        ...
        from_date = from_date + datetime.timedelta(days=CLASSES_SCHEDULE_DAYS_IN_SINGLE_BATCH)
    if days is not None:
